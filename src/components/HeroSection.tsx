import heroImage from "@/assets/hero-kitchen.jpg";
import { ArrowRight } from "lucide-react";

const HeroSection = () => {
  return (
    <section className="pt-24 pb-20 md:pt-36 md:pb-32 px-5 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          {/* Text */}
          <div className="animate-fade-in">
            <span className="inline-block text-xs font-semibold tracking-widest uppercase text-accent bg-accent/10 px-4 py-2 rounded-full mb-8">
              Trusted builders in London & South East
            </span>
            <h1 className="font-heading text-5xl md:text-6xl lg:text-7xl text-foreground mb-6 leading-[1.05] tracking-tight">
              Quality building work you can{" "}
              <span className="relative inline-block">
                rely on
                <span className="absolute -bottom-2 left-0 w-full h-3 bg-accent/20 rounded-full -z-10" />
              </span>
            </h1>
            <p className="text-muted-foreground text-lg md:text-xl leading-relaxed mb-10 max-w-lg">
              From new builds to bathroom refits — we handle every trade in-house so your project runs smoothly, on time, and on budget.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <a
                href="#contact"
                className="group inline-flex items-center justify-center gap-2 px-8 py-4 bg-primary text-primary-foreground text-sm font-semibold rounded-2xl shadow-[0_8px_30px_-6px_hsl(var(--primary)/0.4)] hover:shadow-[0_14px_40px_-4px_hsl(var(--primary)/0.5)] hover:-translate-y-0.5 transition-all duration-300"
              >
                Get a free quote
                <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform duration-200" />
              </a>
              <a
                href="#projects"
                className="inline-flex items-center justify-center gap-2 px-8 py-4 border-2 border-border text-foreground text-sm font-semibold rounded-2xl hover:border-primary/30 hover:bg-primary/[0.03] transition-all duration-300"
              >
                View our work
              </a>
            </div>

            {/* Trust strip */}
            <div className="mt-14 flex items-center gap-10 text-sm text-muted-foreground">
              {[
                { label: "15+ years", value: "Experience" },
                { label: "200+", value: "Projects" },
                { label: "100%", value: "Insured" },
              ].map((stat, i) => (
                <div key={i} className="text-center">
                  <p className="font-heading text-2xl text-foreground">{stat.label}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{stat.value}</p>
                </div>
              ))}
            </div>
          </div>

          {/* Image */}
          <div className="animate-scale-in">
            <div className="relative">
              <div className="absolute -inset-4 bg-accent/5 rounded-[2rem] -rotate-3" />
              <img
                src={heroImage}
                alt="Modern kitchen renovation with exposed brick and copper fixtures"
                className="relative w-full aspect-[4/3] object-cover rounded-3xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.15)]"
                loading="eager"
              />
              <div className="absolute -bottom-6 -left-6 bg-background border border-border backdrop-blur-sm px-6 py-4 rounded-2xl shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)]">
                <p className="text-3xl font-heading text-foreground">100%</p>
                <p className="text-xs text-muted-foreground">Satisfaction rate</p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default HeroSection;
